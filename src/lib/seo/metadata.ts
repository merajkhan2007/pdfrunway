 ...
 */
export function generateCookieMetadata(locale: Locale, translations?: { title: string; description: string }): Metadata {
  return generateBaseMetadata({
    locale,
    path: '/cookies',
    title: translations?.title || 'Cookie Policy',
    description: translations?.description || `Learn about how ${siteConfig.name} uses cookies to improve user experience and analyze website performance.`,
    keywords: ['cookies', 'cookie policy', 'privacy', 'local storage'],
  });
}
